...
def patch_repositories(monkeypatch):
    ...
    class FakeVehicleRepo:
        ...
        def create(self, vehicle):
            v = _Vehicle(self._seq, vehicle.plate)
    ...
    class FakeAccessRepo:
        ...
        def find_open_by_vehicle(self, lot_id: int, vehicle_id: int):
            if self._open and self._open.parking_lot_id == lot_id and self._open.vehicle_id == vehicle_id and self._open.status == "open":
                return self._open
            return None

        def create(self, access):
            a = _Access(self._seq, access.vehicle_id, access.parking_lot_id, access.start_at)
            self._open = a
            self._seq += 1
            return a
